...
describe('resumida quote PDF', () => {
  it('renders a valid concise PDF under Node with intentional image fallbacks', async () => {
    const consoleError = vi.spyOn(console, 'error').mockImplementation(() => undefined)

    const buffer = await renderResumidaQuotePdf(representativeSnapshot)
  ...
  it('keeps many long pt-BR product rows legible across multiple pages', async () => {
    const items = Array.from({ length: 28 }, (_, index) => ({
      ...
      description: `${representativeSnapshot.items[0].description} — variação ${index + 1} com conteúdo acentuado`,
    }))
    const snapshot: QuotePdfSnapshot = {
      ...
      items,
      notes: `${representativeSnapshot.notes} `.repeat(8),
    }
  ...
  it('uses no browser APIs, remote assets, or financial calculation engine', () => {
    ...
    const source = readFileSync(
      new URL('../../src/lib/pdf/resumida.server.tsx', import.meta.url),
      'utf8',
    )
