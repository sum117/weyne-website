  ...
  return (
    <main className="min-w-0 flex-1 px-4 py-8 sm:px-6 nav:px-8" id="app-main">
      <section className="mx-auto min-w-0 w-full max-w-xl rounded-lg border border-border bg-card p-6 sm:p-10">
        ...
        <p
          className="mt-3 break-all font-sans text-sm text-muted-foreground"
          data-server-rendered-at={status.renderedAt}
        >
          Resposta SSR gerada em {status.renderedAt}.
        </p>
        <p className="mt-3 break-words font-sans text-sm text-muted-foreground" data-session-email>
          Sessão ativa: {session.user.email}.
        </p>
        <div className="mt-8 flex flex-wrap items-center gap-4">
          <Link
            to="/"
            className="inline-flex min-h-11 items-center font-sans font-semibold text-primary underline decoration-secondary decoration-2 underline-offset-4"
          >
